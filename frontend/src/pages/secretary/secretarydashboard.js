...
const SecretaryDashboard = () => {
  ...
  const generateMonthlyReport = () => {
    ...
    doc.text('Community Fund Management', 14, 26);
    doc.text(`Monthly Report — ${monthName} ${year}`, 14, 34);

    doc.setTextColor(0, 0, 0);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text(`Fund: ${fund?.name || 'N/A'}`, 14, 52);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 100, 100);
    doc.text(`Generated by Secretary on ${now.toLocaleDateString()}`, 14, 60);
    doc.text(`Total Balance: ${(fund?.totalBalance || 0).toLocaleString()} RWF`, 14, 68);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text('Summary', 14, 83);

    const contribs = transactions.filter(t => t.type === 'contribution' && t.status === 'approved');
    const loans = transactions.filter(t => t.type === 'loan' && t.status === 'approved');
    ...
    doc.autoTable({
      ...
      head: [['Metric', 'Value']],
      body: [
        ['Total Contributions', `${totalContrib.toLocaleString()} RWF`],
        ['Total Loans Approved', `${totalLoans.toLocaleString()} RWF`],
        ['Pending Requests', pending.length.toString()],
        ['Total Members', members.length.toString()],
        ['Fund Balance', `${(fund?.totalBalance || 0).toLocaleString()} RWF`],
      ],
      theme: 'grid',
   ...
   const membersY = doc.previousAutoTable.finalY + 12;
    ...
    doc.autoTable({
      ...
      body: members.map(m => [
        ...
        (m.totalContributed || 0).toLocaleString()
      ]),
      theme: 'striped',
      headStyles: { fillColor: [10, 11, 14], textColor: [201, 168, 76], fontStyle: 'bold' },
      alternateRowStyles: { fillColor: [250, 250, 250] },
    ...
    doc.save(`IBIMINA_Report_${monthName}_${year}.pdf`);
  };
          ...
          <div className="page-header">
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button className="btn btn-outline" onClick={generateMonthlyReport}>
